...
export class PetModel {
  constructor(type = PetTypes.CANINE) {
    this.petId = uuidv4();
    this.ownerId = null;
    this.petType = type;
    ...
    this.attributes = {
      ...
      age: 0,
      // Additional attributes for detailed state tracking
      energy: 1.0,
      happiness: 1.0,
      nutrition: 1.0,
      hydration: 1.0,
  ...
  calculatePhysicalHealth() {
    const { exercise, nutrition, hydration, energy } = this.attributes;
    const healthIndex = (exercise * 0.4) + (nutrition * 0.3) + 
                       (hydration * 0.2) + (energy * 0.1);
  ...
  calculateEmotionalState() {
    ...
    const emotionalIndex = (happiness * 0.4) + (mindfulness * 0.3) + 
  ...
  checkHealth() {
    ...
    if (this.attributes.physicalHealth < healthThreshold || 
  ...
  updatePersonality() {
    const dominantScore = Math.max(
      ...Object.values(this.stats.personalityScores)
    );
    const dominantType = Object.entries(this.stats.personalityScores)
      .find(([_, score]) => score === dominantScore)?.[0];
